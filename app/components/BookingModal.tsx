...
export default function BookingModal({ isOpen, onClose, initialService }: BookingModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialService) {
        const service = SERVICES.find(s => s.title === initialService) || SERVICES[1]
      ...
      setRiders([{ firstName: '', lastName: '', dob: '', email: '', phone: '' }])
      setCurrentYear(2026)
  ...
  // July 2026 details (Jul 1 2026 starts on Wednesday)
  ...
  // Available days: match July 30 and July 31 2026 from the mockups.
  // For other months, let's make all weekend days and the last two days of the month available to keep the calendar interactive!
  const isDateAvailable = (day: number) => {
    if (currentYear === 2026 && currentMonth === 6) {
      return day === 30 || day === 31
    }
    // General availability logic for mockup: weekends + last two days
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-6 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full h-full md:h-auto md:max-h-[95vh] md:max-w-4xl bg-[#fafaf9] md:rounded-3xl shadow-2xl flex flex-col overflow-hidden text-stone-800">
        
        {/* Header Bar */}
        <div className="px-6 py-4 flex items-center justify-between border-b border-stone-200 bg-white">
          <span className="font-bold text-lg text-stone-800 font-serif">
            {step === 0 ? 'Book a Ride' : selectedService?.title}
          </span>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8">
          ...
          {step === 0 && (
            <div className="max-w-3xl mx-auto text-center">
              ...
              <p className="text-stone-500 font-medium tracking-wide uppercase text-xs mb-8">Our Services</p>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-2 space-y-6">
                  {riders.map((rider, index) => (
                    <div key={index} className="bg-white p-6 rounded-2xl border border-stone-200 relative">
          ...
          {step === 4 && (
            <div className="max-w-md mx-auto text-center py-10 px-4">
              ...
              <button 
                onClick={onClose}
                className="w-full bg-[#5c3d35] hover:bg-[#482e28] text-white font-semibold py-3 rounded-xl transition shadow-lg hover:shadow-xl font-serif"
              >
                Return to Homepage
              </button>
            </div>
        ...
        {/* MODAL FOOTER BAR (Only visible for steps 1-3) */}
        {step > 0 && step < 4 && (
          <div className="px-6 py-4 border-t border-stone-200 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
